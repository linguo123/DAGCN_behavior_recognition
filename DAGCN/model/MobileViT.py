...
def conv_bn(inp,oup,kernel_size=3,stride=1):
    return nn.Sequential(
        nn.Conv2d(inp,oup,kernel_size=kernel_size,stride=stride,padding=(kernel_size//2,1)),
        nn.BatchNorm2d(oup),
        nn.SiLU()
    )
...
class MobileViTAttention(nn.Module):
    def __init__(self,in_channel=3,dim=512,kernel_size=3,patch_size=7,depth=3,mlp_dim=1024):
        ...
        self.conv1=nn.Conv2d(in_channel,in_channel,kernel_size=kernel_size,padding=(kernel_size//2,1))
        self.conv2=nn.Conv2d(in_channel,dim,kernel_size=1)

        self.trans=Transformer(dim=dim,depth=depth,heads=8,head_dim=64,mlp_dim=mlp_dim)

        self.conv3=nn.Conv2d(dim,in_channel,kernel_size=1)
        self.conv4=nn.Conv2d(2*in_channel,in_channel,kernel_size=kernel_size,padding=(kernel_size//2,1))
...
class MobileViT(nn.Module):
    def __init__(self,image_size,dims,channels,num_classes,depths=[2,4,3],expansion=4,kernel_size=3,patch_size=[30,5]):
        super().__init__()
        ih,iw=image_size[0],image_size[1]
        ph,pw=patch_size[0],patch_size[1]
        assert iw%pw==0 and ih%ph==0

        self.conv1=conv_bn(3,channels[0],kernel_size=3,stride=(2,1))
        ...
        self.mv2.append(MV2Block(channels[1],channels[1],2))
        self.m_vits.append(MobileViTAttention(channels[1],dim=dims[0],kernel_size=kernel_size,patch_size=patch_size,depth=depths[0],mlp_dim=int(2*dims[0])))
        self.mv2.append(MV2Block(channels[1],channels[2],2))
        self.m_vits.append(MobileViTAttention(channels[2],dim=dims[1],kernel_size=kernel_size,patch_size=patch_size,depth=depths[1],mlp_dim=int(4*dims[1])))
        #self.mv2.append(MV2Block(channels[3],channels[4],2))
        #self.m_vits.append(MobileViTAttention(channels[4],dim=dims[2],kernel_size=kernel_size,patch_size=patch_size,depth=depths[2],mlp_dim=int(4*dims[2])))

        
        self.conv2=conv_bn(channels[-2],channels[-1],kernel_size=1)
        self.pool=nn.AvgPool2d(image_size[0]//32,1)
        self.fc=nn.Linear(channels[-1],num_classes,bias=False)
        nn.init.normal_(self.fc.weight, 0, math.sqrt(2. / 60))
...
def mobilevit_xxs():
    dims=[10,30]
    channels= [3, 16,128]
    return MobileViT([300,25],dims,channels,num_classes=60)
