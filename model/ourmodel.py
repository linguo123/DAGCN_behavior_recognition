...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, coff_embedding=4, num_subset=3):
        ...
        num_jpts = A.shape[-1]
        self.Graph = torch.tensor(Graph('spatial').get_adjacency_matrix(), dtype=torch.float32, requires_grad=False).to(
            'cuda')
        ...
        self.conv_c = nn.Conv2d(out_channels + 3, out_channels, 1)
    ...
    def forward(self, x0):
        ...
        norm_learn_A = self.conv_c(torch.cat((A1, self.Graph), dim=0))
