...
class CTRGC(nn.Module):
    def __init__(self, in_channels, out_channels, rel_reduction=4, mid_reduction=1):
        ...
        self.fc_H = nn.Linear(self.rel_channels, self.rel_channels)
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        # x_b = x_b[:, :, ::2, :]
        N, C, T, V = x_a.size()
        A1 = x_a.view(N, self.rel_channels, 8, T, V).permute(0, 1, 4, 2, 3).contiguous().view(N, self.rel_channels, V,
                                                                                              8 * T)
        A2 = x_b.view(N, self.rel_channels, 8, T, V).permute(0, 1, 2, 3, 4).contiguous().view(N, self.rel_channels,
                                                                                              8 * T, V)
        A1 = self.tanh(torch.matmul(A1, A2) / A1.size(-1))  # N C V V
        H_ = A1.view(N, self.rel_channels, -1).mean(-1)
        H_att = self.tanh(self.fc_H(H_)).unsqueeze(2).unsqueeze(3)
        # 新增的方法
        A = A[1:3].unsqueeze(0).repeat(
            1, self.rel_channels // 2, 1, 1)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True, kernel_size=5,
        ...
        self.tcn1 = unit_tcn(out_channels, out_channels, kernel_size=1,
                             stride=stride)  # MultiScale_TemporalConv(out_channels, out_channels, kernel_size=kernel_size, stride=stride, dilations=dilations,
        # residual=False)
        self.relu = nn.ReLU(inplace=True)
        if not residual:
            self.residual = lambda x: 0
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc(x), self.fc(x + x_1 + x_2), self.fc(x + x_2)