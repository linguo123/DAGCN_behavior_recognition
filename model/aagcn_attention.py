...
class De_unit_gcn(nn.Module):
    ...
    def forward(self, x0):
        ...
        A2 = self.conv_a(x0).permute(1,0, 2, 3).contiguous().view(self.out_channels,N * T,V)
        ...
        norm_learn_A = norm_learn_A+ A1*self.alpha
...
class unit_gcn(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, A, groups, num_point, coff_embedding=4, num_subset=3, adaptive=True, attention=True):
    ...
    def forward(self, x):
        ...
        if self.adaptive:
            ...
            # A = A + self.PA
            for i in range(self.num_subset):
                A1 = self.conv_a[i](x).permute(1, 3, 0, 2).contiguous().view(self.inter_c , V, N* T)
                A2 = self.conv_b[i](x).permute(1, 0, 2, 3).contiguous().view(self.inter_c ,N * T, V)
                A1 = self.tan(torch.matmul(A1, A2) / A1.size(-1))  # out_channels * num_subset V V
                A1 = A[i] + A1 * self.alpha
                A1 = norm_learn_A[i] + A1 *self.alpha2
                A2 = x.view(N, C * T, V)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A,  groups, num_point,stride=1, residual=True, adaptive=True, attention=True):
        super(TCN_GCN_unit, self).__init__()
        self.gcn1 = unit_gcn(in_channels, out_channels, A, groups, num_point, adaptive=adaptive, attention=attention)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, groups=16, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l1 = TCN_GCN_unit(3, 64, A,groups, num_point, residual=False, adaptive=adaptive, attention=attention)
        self.l2 = TCN_GCN_unit(64, 64, A,groups, num_point, adaptive=adaptive, attention=attention)
        self.l3 = TCN_GCN_unit(64, 64, A,groups, num_point, adaptive=adaptive, attention=attention)
        self.l4 = TCN_GCN_unit(64, 64, A,groups, num_point, adaptive=adaptive, attention=attention)
        self.l5 = TCN_GCN_unit(64, 128, A,groups, num_point, stride=2, adaptive=adaptive, attention=attention)
        self.l6 = TCN_GCN_unit(128, 128, A,groups, num_point, adaptive=adaptive, attention=attention)
        self.l7 = TCN_GCN_unit(128, 128, A,groups, num_point, adaptive=adaptive, attention=attention)
        self.l8 = TCN_GCN_unit(128, 256, A,groups, num_point, stride=2, adaptive=adaptive, attention=attention)
        self.l9 = TCN_GCN_unit(256, 256, A,groups, num_point, adaptive=adaptive, attention=attention)
        self.l10 = TCN_GCN_unit(256, 256, A,groups, num_point, adaptive=adaptive, attention=attention)
