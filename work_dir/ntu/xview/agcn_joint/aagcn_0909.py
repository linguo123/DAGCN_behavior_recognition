...
class unit_gcn(nn.Module):
    ...
    def forward(self, x0):
        ...
        A1 = self.tan(torch.matmul(A1, A2) / A1.size(-1))#.unsqueeze(0).repeat(3,1,1,1)
        #     1, self.out_channels // self.groups, 1, 1))
...
class unit_gcn(nn.Module):
    ...
    def forward(self, x0):
        ...
        A  = self.conv_a(x0)
        A1 =A.permute(1,3,0,2).contiguous().view(self.out_channels,V,N*T)
        A2 = A.permute(1,0, 2, 3).contiguous().view(self.out_channels,N * T,V)
        ...
        # x = x.view(n, self.num_subset, kc // self.num_subset, t, v)
        x = torch.einsum('ndtv,dvw->ndtw', (A, norm_learn_A))
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, num_point, groups, stride=1, residual=True, adaptive=True, attention=True):
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, attention=True,groups = 8):
        ...
        self.l1 = TCN_GCN_unit(3, 64, A, num_point,groups,residual=False, adaptive=adaptive, attention=attention)
        self.l2 = TCN_GCN_unit(64, 64, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.l3 = TCN_GCN_unit(64, 64, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.l4 = TCN_GCN_unit(64, 64, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.l5 = TCN_GCN_unit(64, 128, A, num_point,groups,stride=2, adaptive=adaptive, attention=attention)
        self.l6 = TCN_GCN_unit(128, 128, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.l7 = TCN_GCN_unit(128, 128, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.l8 = TCN_GCN_unit(128, 256, A, num_point,groups,stride=2, adaptive=adaptive, attention=attention)
        self.l9 = TCN_GCN_unit(256, 256, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.l10 = TCN_GCN_unit(256, 256, A, num_point,groups,adaptive=adaptive, attention=attention)
        self.dec_T = torch.nn.Conv2d(in_channels=256,out_channels=256,kernel_size=(75,1))
        self.dec_V = torch.nn.Conv2d(in_channels=256,out_channels=256,kernel_size=(1,25))
    ...
    def forward(self, x):
        ...
        # N*M,C,T,V
        c_new = x.size(1)
        x_T =  self.relu(self.bn(self.dec_T(x)).squeeze(-1))
        x_V =  self.relu(self.bn(self.dec_V(x)).squeeze(-1))
        x_T = x_T.view(N, M, c_new, -1).mean(3).mean(1)
        # x_T = self.dec_T(x_T).squeeze(-1)
        x_V = x_V.view(N, M, c_new, -1).mean(3).mean(1)
        # x_V = self.dec_V(x_V).squeeze(-1)
        ...
        return self.fc(x),self.fc(x_T),self.fc(x_V)