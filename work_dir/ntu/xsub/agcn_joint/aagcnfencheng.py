...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=1, adaptive=True, attention=True):
    ...
    def forward(self, x):
        ...
        if self.adaptive:
            ...
            for i in range(3):
                ...
                A2 = x.view(N, C * T, V)
                z = self.conv_d[0](torch.matmul(A2, A1).view(N, C, T, V))
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x):
        ...
        # x1 ,x2 ,x3 = torch.split(x,[25,5,1],dim=3)
        # x_list = [x1 ,x2 ,x3]
        # y_list = [ ]
        ...
        if self.attention:
            ...
            y = self.relu(self.tcn1(self.gcn1(x)) + self.residual(x))
             # y_list.append(y)
        return y # torch.cat(y_list,dim=3)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.H1 = nn.Conv2d(256,256, kernel_size=(1,3), padding=(0, 1), stride=(1, 3))
        self.H2 = nn.Conv2d(256, 256, kernel_size=(1, 5), padding=(0, 2), stride=(1, 5))
        self.H3 = nn.Conv2d(256, 256, kernel_size=(1, 25), padding=(0, 0), stride=(1, 1), )
        ...
        kernel_size_list = [25,5,1]
        for i in range(3):
            self.dec_T.append(nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(38, 1)))
            self.dec_V.append(nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(kernel_size_list[i], 1)))
        ...
        self.fc2 = nn.Linear(256*3, num_class)
        self.bn2 = nn.BatchNorm2d(256*3)
    ...
    def forward(self, x):
        ...
        x_V_list = []
        for i in range(3):
            x_T = self.relu(self.bn(self.dec_T[i](x_list[i])))
            x_V = self.relu(self.bn(self.dec_V[i](x_list[i])))
            x_T = x_T.view(N, M, c_new, -1).mean(3).mean(1)
            # x_T_V = x_T.view(N, M, c_new, -1).mean(3).mean(1)
        ...
        # x_T = torch.cat([x_T_list[0], x_T_list[1], x_T_list[2]], dim=1)
        # x_V = torch.cat([x_V_list[0], x_V_list[1], x_V_list[2]], dim=1)



        x1 = x_T_list[0]+x_V_list[0]
        x2 = x_T_list[1]+x_V_list[1]
        # x3 = self.bn1(x_T_list[2]+x_V_list[2]+x)
        # x = self.drop_out(x)

        # return self.fc(x1), self.fc(x), self.fc( x_T+x_V)
        return self.fc(x), self.fc(x1), self.fc(x2)